...
void send_BIGWRQ(const char *fileName, int sockfd, struct sockaddr_in addr) {
    char buffer[SIZE];
    int n, readBytes, blockNumber = 0, retries;
    socklen_t addr_size = sizeof(struct sockaddr_in); // Déclaration de addr_size
    ...
    buffer[1] = 2; // Opcode WRQ
    strcpy(buffer + 2, fileName); // Nom du fichier
    strcpy(buffer + 2 + strlen(fileName) + 1, "octet"); // Mode ("octet")
    strcpy(buffer + 2 + strlen(fileName) + 1 + strlen("octet") + 1, "bigfile"); // Option "bigfile"
    ...
    // Attente et vérification de la requête WRQ bigfile
    ...
    if (n < 0) {
        if (errno == EWOULDBLOCK || errno == EAGAIN) {
            printf("[ERROR] Timeout occurred while waiting for ACK.\n");
            fclose(fp);
            exit(EXIT_FAILURE); // Sortir ou renvoyer -1 en fonction de la logique de votre application
        } else {
            fclose(fp);
            dieWithError("[ERROR] receiving ACK");
        }
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 3) {
    strcpy(option, argv[3]);  // Copie de l'option
    } else {
    // Aucune option fournie, vous pouvez choisir de laisser l'option vide ou d'utiliser une valeur par défaut
    // Par exemple :
        strcpy(option, ""); // Laisser l'option vide
    // OU
    // strcpy(option, "default_option"); // Utiliser une valeur par défaut
    }
